...
class ResNet(nn.Module):
    ...
    def forward(self, x, beta, gamma, test=False):

        x_conv1 = self.conv1(x)        
        x_variation = beta*x_conv1 + gamma # added variation
        if test:
            if not isinstance(beta, int) and not isinstance(gamma, int):
                #print_features(x_conv1, x_variation, beta, gamma)
                beta_np = beta.cpu().numpy().reshape((64,112,112))
                gamma_np = gamma.cpu().numpy().reshape((64,112,112))
                f_conv1_np = x_conv1.cpu().numpy().reshape((64,112,112))
                f_variation_np = x_variation.cpu().numpy().reshape((64,112,112))
        
        x = self.bn1(x_variation)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        #x = beta*x + gamma # added variation
        ...
        if test and not isinstance(beta, int): #for printing the features
            if self.noBN:
                return x, conv_layer4BN, conv_layer4NBN, beta_np, gamma_np, f_conv1_np, f_variation_np
            else:
                return x, conv_layer4BN, beta_np, gamma_np, f_conv1_np, f_variation_np
        else:        
            if self.noBN:
    ...
    def get_features():
        ...
        g = np.array(self.gammas)
        f_c = np.array(self.f_conv1)
        f_v = no.array(self.f_variation)
