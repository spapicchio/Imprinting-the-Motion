...
class MyConvLSTMCell(nn.Module):
    ...
    def forward(self, x, state):
        ...
        ht_1, ct_1 = state
        it = torch.sigmoid(self.conv_i_xx(x) + self.conv_i_hh(ht_1)) #changed from F.sigmoid
        ft = torch.sigmoid(self.conv_f_xx(x) + self.conv_f_hh(ht_1)) #changed from F.sigmoid
        ct_tilde = torch.tanh(self.conv_c_xx(x) + self.conv_c_hh(ht_1)) #changed from F.tanh
        ct = (ct_tilde * it) + (ct_1 * ft)
        ot = torch.sigmoid(self.conv_o_xx(x) + self.conv_o_hh(ht_1)) #changed from F.sigmoid
        ht = ot * torch.tanh(ct) #changed from F.tanh
        return ht, ct